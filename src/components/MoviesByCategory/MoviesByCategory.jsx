...
import { countries, genres, moviesByFilters as movies} from './delete.data'
...
export default function MoviesByCategory() {
   ...
   const filtersData = {
      years: Array(46).fill(null).map((__, index) => index+1980),
      rating: Array(9).fill(null).map((__, index) => index+1),
      genres,
      countries,
   }
   ...
   const {movies, error, loading} = useFetchMoviesByFilters(selectedFilters, 8)
  ...
  return (
    <div className={styles.moviesByCategory}>
      ...
      <Filters filtersData={filtersData} selectedFilters={selectedFilters} setSelectedFilters={setSelectedFilters}/>
