...
export function useFetchMoviesByFilters(
  { year, rating, genre, country },
  limit,
) {
    ...
    async function fetchData() {
      try {
        ...
        const response = await fetch(
          `https://api.kinopoisk.dev/v1.4/movie?page=1&limit=${limit}&notNullFields=name&notNullFields=top250&notNullFields=description&notNullFields=year&notNullFields=countries.name&notNullFields=genres.name&notNullFields=ageRating&notNullFields=rating.kp&notNullFields=poster.url${yearPath}${ratingPath}${genrePath}${countryPath}`,
          {
            method: "GET",
            headers: {
              "X-API-KEY": API_KEY,
